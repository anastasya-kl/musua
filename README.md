...
3. [Технічна реалізація](#technical-realization)
  3.1. [Проектування бази даних](#database-design)
  3.2. [Оформлення кодової частини](#code)
  3.3. [Аналіз backend-у](#backend)
  3.4. [Аналіз API](#api)
  3.5. [Аналіз frontend-у](#frontend)
4. [Готовий додаток](#full-stacked-app)

> [!NOTE]
> Цей проект не є готовим рішенням, що враховує усі нюанси процесу створення та підтримки подібного роду платформ. Він детально відображає лише ключові кроки розробки та може слугувати базою для подальшого розвитку.

---
## :memo: <span id="description" >1. Опис</span>

Додаток надає можлиливість ___прослуховувати треки українських виконавців і взаємодіяти із контентом___ - зберігати у обране та створювати власні плейлисти. Він містить _повну інформацію про пісні та виконавців_ - назви, жанри, роки випуску, альбоми та їх обкладинки, аудіо контент, популярність. Ці дані використовуються для вирішення ще одного базового завдання додатку - __підбір персональної системи рекомендацій користувача на основі його вподобань__. Для цього у базі також зберігаються особисті дані, такі як нікнейм, емейл, пароль, вподобаний контент та створені плейлисти.
...
Задача формування рекомендацій на основі вподобань у даному випадку не є складною задачею і може включати у себе просту фільтрацію на основі схожості контенту.

А от процес кластеризації потребує більш детального аналізу. Виведемо основні моменти:
...
Найпоширенішими варіантами алгоритмів кластеризації є:
...
### <span id="database-design">Проектування бази даних</span>

Проаналізуємо, які таблиці були створені, їхні зв'язки та власне дані, які вони зберігають

#### <span id="music-content">3.1. Дані про музичний контент</span>
...
2. __`albums`__ - альбоми. Містить унікальний ідентифікатор, датe випуску, загальну кількість треків, _тип_ та популярність. Під _типом_ тут мається на увазі чи це власне альбом, чи сингл. album може мати декілька виконавців і тоді виникає проблема взаємозв'язоку `"багато до багатьох" (M:M)`, тому для цььго тут створена проміжна таблиця `artist_albums`.
> [!WARNING]
...
5. __`categories`__ - категорії. Містить назву (наприклад, _Рок_), та узагальнену (просту) назву жанру (наприклад, _rock_). У категоріях зберігатимуться плейлисти, альбоми та акаунти артистів загалом, що підпадають під певний жанр музики.
...
1. __`playlists`__ - плейлисти. Має поля назва, опис, обкладинка, дата створення. У даному випадку наші плейлисти можуть бути створені Musua як публічні і зберігатися упевних категоріях контенту; можуть бути також створені Musua, але на основі смаків користувача і  взаємодіяти із ним надаватиметься лише йому; і, звичайно, плейлисти можуть створювати самі користувачі. Для таких потреб тут додані поля у вигляді FK на id користувача - `id_user`, якщо цей плейлист _створений користувачем_ та `for_id_user`, якщо _плейлист створений для користувача_. 
...
##### content_data.py
`content_data.py` також містить функції, що дістають дані, але ця інформація є більший комплексною і потрібна для створення рекомендаційного контенту - наприклад, дістати 100 найпопулярніших виконавців, повернути пісні, що написані у жанрі рок тощо.
...
##### filler.py
`filler.py` - функції заповнення музичного контенту.
...
У пункті [Теоретична база формування рекомендацій](#teoretical-reccomendations) ми вже проаналізували різні алгоритми кластеризації, їхні недоліки та визначили найкращий варіант - `OPTICS`. Нижче наведена функція, яка
...
> [!TIP]
> Алгоритми машинного навчання, включаючи `OPTICS`, працюють з _числовими даними_. ID артистів, у нашому випадку, є текстовими рядками, а не числовими ідентифікаторами - їх ID відповідають тим ID, які зберігаються у `Spotify`, і саме їх ми отримали через `SpotifyAPI`. 
>
>Такий спосіб зберігання даних є оптимальним рішенням із розрахунку на майбутнє (наприклад, при обновленні інформації про артистів - добавити новий альбом, змінити фото тощо), оскільки у БД напряму зберігаються валідні для запиту ідентифікатори. 

> [!NOTE]
>Не лише виконавці мають рядковий ідентифікатор - інші дані, такі як _пісні та альбоми_, інформацію про які теж було здобуто через `SpotifyAPI`, мають такого типу ID.
...
> - Усунення шуму
> Квантилі допомагають зрозуміти розподіл даних, виявити ці викиди та усунути їх.
...
>  - `OPTICS`, як і багато інших алгоритмів машинного навчання, очікує, що __всі вхідні вектори__ (в нашому випадку - списки артистів для кожного користувача) матимуть __однакову довжину__. 
...
Ця функція повертає улюблені пісні користувача із заданим `user_id`:
  ...
  - далі у до `функції endpoint-у` передається як аргумент `user_id`.
...
> Дизайн frontend частини засобами ReactNativeJS був створений розробником [LymarVictoriia](https://github.com/LymarVictoriia), тому тут ми розглянемо не принципи побудови компонентів інтерфейсу, а власне організацію робити API на стороні клієнта.
